...
function initexec(modulename,command) {
	...
	for (var asset in global.hybridd.asset) {
      ...
      modules.module[global.hybridd.asset[target.name].module].main.exec({processID:processID,target:target,mode:mode,factor:factor,command:command});
		}
	}
	...
	for (var source in global.hybridd.source) {
      ...
      modules.module[global.hybridd.source[target.name].module].main.exec({processID:processID,target:target,mode:mode,factor:factor,command:command});  
		}
	}
}
