...
function initexec(modulename,command) {
	...
	for (var asset in global.hybridd.asset) {
      ...
      modules.module[global.hybridd.asset[target.name].module].main.exec({processID,target,mode,factor,command});
		}
	}
	...
	for (var source in global.hybridd.source) {
      ...
      modules.module[global.hybridd.source[target.name].module].main.exec({processID,target,mode,type,command});  
		}
	}
}
